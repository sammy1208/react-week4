...
今天主要處理長篇小說資料整理、翻譯工作流、站內小說資料結構，以及閱讀頁後續維護風險。

### 長篇小說整理

針對 `【佐久侑】失恋holiday` 這種由 Notion 匯出後拆成多個章節檔的長篇，建立統一處理方式：

- 確認主檔 `【佐久侑】失恋holiday.md` 實際上是章節索引。
- 找出完整章節來源：
  - 第一章
  - 第二章
  - 第三章
  ...
  - 番外1：八卦小報
  - 番外2：乾咖哩
- 將分章檔合併成單一正式 Markdown：
  - `docs/待確認/【佐久侑】失戀holiday_合併.md`
- 統一整理成站內小說格式：
  - frontmatter
  - `### Chapter N`
  - `### Extra N`
  ...
  - `<div>...</div>`
  - `<!--/chapter content-->`
- 將純文字段落包成 `<p>`，避免和其他小說檔格式不一致。
- 保留原本分章檔，方便後續人工對照與校稿。

檢查結果：

- 共 8 個章節區塊。
- `chapter content` 開關標記數量一致。
- `<div>` 數量平衡。
- `<p>` 數量平衡。
- 沒有 4 空白縮排造成 Markdown 將 HTML 誤判為程式碼。

### 翻譯與校稿工作

延續 `docs/修改手冊.md` 與 `docs/參考資料/haikyuu-reference.md` 的規則，處理多篇排球少年 / 佐久侑文章：

- 翻譯與整理 `The_Queen_and_the.html`。
- 產出 `docs/down/10000up/The_Queen_and_the.md`。
- 檢查多章結構、AO3 外殼、HTML 標籤平衡與縮排問題。
- 依照角色參考資料修正常見錯名與暱稱：
  - 侑
  - 阿侑
  - 治
  ...
  - 臣臣
  - 倫
- 保留 `<p>`、`<em>`、`<strong>` 等原文格式標籤，避免破壞閱讀頁渲染。

### 偏好風格分析與試寫

依據 `src/data/SakuAtsu.json` 中標記為 `"珍藏"` 的作品，分析目前偏好的佐久侑故事類型：

- 生活感強，重視日常細節。
- 情感推進要藏在互動、照顧、鬥嘴與身體距離裡。
- 可以有成人張力，但核心仍要回到角色關係。
- 偏好慢熱、曖昧、錯位與嘴硬照顧。
- AU 可以接受，但角色本質要穩。
...
內容走職業篇黑狼日常，主軸是宮侑感冒、佐久早送他回家並煮味噌湯，測試站內偏好的生活感與關係氛圍。
...
- CP 清單數量。
- 目錄中登記的 Markdown 是否存在。
- Markdown 與加密輸出是否缺漏。
- 前端找不到小說資料時可以更早在本機發現問題。

### 前端閱讀體驗與資料載入

針對未來小說數量增加的風險，調整方向如下：

- 將大量小說內容拆到 `public/novels/encrypted`，避免全部包進前端 bundle。
- 改由前端按需載入小說內容。
- 建立「載入更多」機制，避免文章列表一次渲染過多資料。
- 修正從 BookPage 麵包屑回到 CompanionPage 時找不到資料的問題。
- 補強 BookPage 載入與錯誤狀態，避免讀取失敗時畫面空白。

### Git 與部署注意事項

- `docs/` 維持本機工作資料夾，不進 Git。
- `public/novels/encrypted/` 若被 `.gitignore` 忽略，GitHub Pages 不會更新小說加密內容。
- 如果只更新目錄 metadata，通常只需要提交 `public/data/novels/*.json`。
- 如果新增或修改小說正文，除了清單之外，也必須確認對應加密輸出是否需要被部署。
- GitHub Pages 若出現 `/src/main.tsx` 404，通常代表部署到的是原始碼或舊輸出，而不是正確 build 後的 `dist`。
